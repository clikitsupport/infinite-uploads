...
use ClikIT\Infinite_Uploads\GuzzleHttp\Psr7\Utils;
...
final class MultipartUploader extends AbstractMultipartUploader
{
    ...
    public function __construct(S3ClientInterface $s3Client, array $requestArgs, string|StreamInterface $source, array $config = [], ?string $uploadId = null, array $parts = [], ?TransferProgressSnapshot $currentSnapshot = null, ?TransferListenerNotifier $listenerNotifier = null)
    {
        ...
        parent::__construct($s3Client, $requestArgs, $config, $uploadId, $parts, $currentSnapshot, $listenerNotifier);
    ...
    protected function createMultipartOperation(): PromiseInterface
    {
        ...
        if ($this->requestChecksum !== null) {
            ...
            $this->isFullObjectChecksum = \true;
        } elseif ($this->config['request_checksum_calculation'] === 'when_supported') {
            $this->requestChecksumAlgorithm = $createMultipartUploadArgs['ChecksumAlgorithm'] ?? self::DEFAULT_CHECKSUM_CALCULATION_ALGORITHM;
            $createMultipartUploadArgs['ChecksumAlgorithm'] = $this->requestChecksumAlgorithm;
        }
        ...
        if (($createMultipartUploadArgs['ChecksumType'] ?? '') === self::CHECKSUM_TYPE_FULL_OBJECT) {
            if (stripos($this->requestChecksumAlgorithm, 'crc') !== 0) {
                return Create::rejectionFor(new S3TransferException("Full object checksum algorithm must be `CRC` family base."));
            }
        }
        $this->operationInitiated($createMultipartUploadArgs);
        $command = $this->s3Client->getCommand('CreateMultipartUpload', $createMultipartUploadArgs);
        return $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) {
    ...
    protected function completeMultipartOperation(): PromiseInterface
    {
        ...
        $completeMultipartUploadArgs['MultipartUpload'] = ['Parts' => $this->parts];
        ...
        if ($this->isFullObjectChecksum && $this->requestChecksum !== null) {
            ...
            $completeMultipartUploadArgs['Checksum' . strtoupper($this->requestChecksumAlgorithm)] = $this->requestChecksum;
        }
        $command = $this->s3Client->getCommand('CompleteMultipartUpload', $completeMultipartUploadArgs);
        return $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) {
    ...
    private function parseBody(string|StreamInterface $source): StreamInterface
    {
        if (is_string($source)) {
            ...
            if (!is_readable($source)) {
                throw new \InvalidArgumentException("The source for this upload must be either a" . " readable file path or a valid stream.");
            }
            ...
            throw new \InvalidArgumentException("The source must be a valid string file path or a StreamInterface.");
        }
        return $body;
    }
    /**
    ...
    private function evaluateCustomChecksum(): void
    {
        ...
        if ($checksumName !== null) {
            ...
            $this->requestChecksumAlgorithm = str_replace('Checksum', '', $checksumName);
    ...
    protected function processMultipartOperation(): PromiseInterface
    {
        ...
        $promises = $this->createUploadPartPromises($uploadPartCommandArgs, $partSize, $partsCount);
    ...
    private function createUploadPartPromises(array $uploadPartCommandArgs, int $partSize, int $partsCount): \Generator
    {
        ...
        $bytesRead = 0;
        $isSeekable = $this->body->isSeekable() && $this->body->getMetadata('wrapper_type') === self::STREAM_WRAPPER_TYPE_PLAIN_FILE;
        while (!$this->body->eof()) {
            if ($isSeekable) {
                $partBody = new LimitStream(new LazyOpenStream($this->body->getMetadata('uri'), 'r'), $partSize, $bytesRead);
            } else {
                $body = new LimitStream($this->body, $partSize, $bytesRead);
            ...
            if ($partNo > self::PART_MAX_NUM) {
                return Create::rejectionFor("The max number of parts has been exceeded. " . "Max = " . self::PART_MAX_NUM);
            }
            if ($isSeekable && $partNo > $partsCount) {
                return Create::rejectionFor("The current part `{$partNo}` is over " . "the expected number of parts `{$partsCount}`");
            }
            ...
            yield $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) use ($command, $partBody) {
                $partBody->close();
                ...
                $this->partCompleted($command['ContentLength'], $command->toArray());
