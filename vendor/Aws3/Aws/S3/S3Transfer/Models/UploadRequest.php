...
final class UploadRequest extends AbstractTransferRequest
{
    ...
    public function validateSource(): void
    {
        if (is_string($this->getSource()) && !is_readable($this->getSource())) {
            throw new InvalidArgumentException("Invalid source `" . $this->getSource() . "` provided. " . "\nPlease provide a valid readable file path or a valid stream as source.");
        }
    }
    /**
    ...
    {
        $requiredParametersWithArgs = ['Bucket' => $this->uploadRequestArgs['Bucket'] ?? null, 'Key' => $this->uploadRequestArgs['Key'] ?? null];
        foreach ($requiredParametersWithArgs as $key => $value) {
            if (empty($value)) {
                ...
                throw new InvalidArgumentException("The `{$key}` parameter must be provided as part of the request arguments.");
            }
        }
    }
}
