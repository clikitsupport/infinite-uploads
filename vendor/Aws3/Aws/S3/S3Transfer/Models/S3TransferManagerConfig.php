...
final class S3TransferManagerConfig
{
    ...
    {
        return new self($config['target_part_size_bytes'] ?? self::DEFAULT_TARGET_PART_SIZE_BYTES, $config['multipart_upload_threshold_bytes'] ?? self::DEFAULT_MULTIPART_UPLOAD_THRESHOLD_BYTES, $config['request_checksum_calculation'] ?? self::DEFAULT_REQUEST_CHECKSUM_CALCULATION, $config['response_checksum_validation'] ?? self::DEFAULT_RESPONSE_CHECKSUM_VALIDATION, $config['multipart_download_type'] ?? self::DEFAULT_MULTIPART_DOWNLOAD_TYPE, $config['concurrency'] ?? self::DEFAULT_CONCURRENCY, $config['track_progress'] ?? self::DEFAULT_TRACK_PROGRESS, $config['default_region'] ?? null);
    }
    /**
     * @return int
     */
