...
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\S3TransferManager;
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Utils\AbstractDownloadHandler;
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Utils\FileDownloadHandler;
...
final class DownloadRequest extends AbstractTransferRequest
{
    ...
    {
        return new self($downloadRequest->getSource(), $downloadRequest->getObjectRequestArgs(), $downloadRequest->getConfig(), $downloadHandler, $downloadRequest->getListeners(), $downloadRequest->getProgressTracker());
    }
    /**
    ...
    public function normalizeSourceAsArray(): array
    {
        ...
        $source = $this->getSource() ?? ['Bucket' => $this->downloadRequestArgs['Bucket'] ?? null, 'Key' => $this->downloadRequestArgs['Key'] ?? null];
        if (is_string($source)) {
        ...
        foreach (['Bucket', 'Key'] as $reqKey) {
            if (empty($sourceAsArray[$reqKey])) {
                throw new \InvalidArgumentException("`{$reqKey}` is required but not provided in " . implode(', ', array_keys($sourceAsArray)) . ".");
            }
        }
