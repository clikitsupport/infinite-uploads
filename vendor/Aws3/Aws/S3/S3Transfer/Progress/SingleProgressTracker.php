...
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Exception\ProgressTrackerException;
/**
...
final class SingleProgressTracker extends AbstractTransferListener implements ProgressTrackerInterface
{
     ...
     */
    public function __construct(ProgressBarInterface $progressBar = new ConsoleProgressBar(), mixed $output = \STDOUT, bool $clear = \true, ?TransferProgressSnapshot $currentSnapshot = null, bool $showProgressOnUpdate = \true)
    {
    ...
    public function transferInitiated(array $context): void
    {
        ...
        $progressFormat->setArg('object_name', $this->currentSnapshot->getIdentifier());
    ...
    public function bytesTransferred(array $context): bool
    {
        ...
        if ($progressFormat instanceof ColoredTransferProgressBarFormat) {
            $progressFormat->setArg('color_code', ColoredTransferProgressBarFormat::BLUE_COLOR_CODE);
        }
        $this->updateProgressBar();
        return \true;
    }
    ...
    public function transferComplete(array $context): void
    {
        ...
        $progressFormat = $this->progressBar->getProgressBarFormat();
        if ($progressFormat instanceof ColoredTransferProgressBarFormat) {
            $progressFormat->setArg('color_code', ColoredTransferProgressBarFormat::GREEN_COLOR_CODE);
        }
        $this->updateProgressBar($this->currentSnapshot->getTotalBytes() === 0);
    }
    /**
     * @inheritDoc
     *
    ...
    public function transferFail(array $context): void
    {
        ...
        $progressFormat = $this->progressBar->getProgressBarFormat();
        if ($progressFormat instanceof ColoredTransferProgressBarFormat) {
            $progressFormat->setArg('color_code', ColoredTransferProgressBarFormat::RED_COLOR_CODE);
            $progressFormat->setArg('message', $context[AbstractTransferListener::REASON_KEY]);
        }
        $this->updateProgressBar();
    }
    /**
    ...
    {
        if (!$forceCompletion) {
            $this->progressBar->setPercentCompleted((int) floor($this->currentSnapshot->ratioTransferred() * 100));
        } else {
            $this->progressBar->setPercentCompleted(100);
        }
        $this->progressBar->getProgressBarFormat()->setArgs(['transferred' => $this->currentSnapshot->getTransferredBytes(), 'to_be_transferred' => $this->currentSnapshot->getTotalBytes(), 'unit' => 'B']);
        // Display progress
    ...
    public function showProgress(): void
    {
        ...
        if ($this->clear) {
            fwrite($this->output, "\x1b[2J\x1b[H");
        }
        fwrite($this->output, sprintf("\r\n%s", $this->progressBar->render()));
        fflush($this->output);
    }
}
