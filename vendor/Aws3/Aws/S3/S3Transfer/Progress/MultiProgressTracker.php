...
final class MultiProgressTracker extends AbstractTransferListener implements ProgressTrackerInterface
{
    ...
    public function transferInitiated(array $context): void
    {
        ...
        if (isset($this->singleProgressTrackers[$snapshot->getIdentifier()])) {
            ...
            if ($this->progressBarFactory === null) {
                $progressTracker = new SingleProgressTracker(output: $this->output, clear: \false, showProgressOnUpdate: \false);
            } else {
                $progressBarFactoryFn = $this->progressBarFactory;
                $progressTracker = new SingleProgressTracker(progressBar: $progressBarFactoryFn(), output: $this->output, clear: \false, showProgressOnUpdate: \false);
            }
    ...
    public function showProgress(): void
    {
        ...
        if (count($this->singleProgressTrackers) !== 0) {
            ...
            $allProgressBarWidth = $this->singleProgressTrackers[$firstKey]->getProgressBar()->getProgressBarWidth();
        }
        $percent = (int) floor($percentsSum / $this->transferCount);
        $multiProgressBarFormat = new MultiProgressBarFormat();
        $multiProgressBarFormat->setArgs(['completed' => $this->completed, 'failed' => $this->failed, 'total' => $this->transferCount]);
        $allTransferProgressBar = new ConsoleProgressBar(progressBarWidth: $allProgressBarWidth, percentCompleted: $percent, progressBarFormat: $multiProgressBarFormat);
        fwrite($this->output, sprintf("\n%s\n", str_repeat('-', $allTransferProgressBar->getProgressBarWidth())));
        fwrite($this->output, sprintf("%s\n", $allTransferProgressBar->render()));
    }
}
