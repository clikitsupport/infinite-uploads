...
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Models\DownloadResult;
...
use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Utils\StreamDownloadHandler;
use ClikIT\Infinite_Uploads\GuzzleHttp\Promise\Coroutine;
use ClikIT\Infinite_Uploads\GuzzleHttp\Promise\Create;
...
abstract class AbstractMultipartDownloader implements PromisorInterface
{
            ...
            try {
                ...
                while ($this->currentPartNo < $this->objectPartsCount) {
                    ...
                    if ($prevPartNo !== $this->currentPartNo - 1) {
                        throw new S3TransferException("Current part `{$this->currentPartNo}` MUST increment.");
                    }
                    $prevPartNo = $this->currentPartNo;
                    $command = $this->nextCommand();
                    yield $this->s3Client->executeAsync($command)->then(function ($result) use ($command) {
                        $this->partDownloadCompleted($result, $command->toArray());
                        return $result;
                ...
                if ($this->currentPartNo !== $this->objectPartsCount) {
                    throw new S3TransferException("Expected number of parts `{$this->objectPartsCount}`" . " to have been transferred but got `{$this->currentPartNo}`.");
                }
                // Transfer completed
                ...
                unset($result['Body']);
                yield Create::promiseFor(new DownloadResult($this->downloadHandler->getHandlerResult(), $result));
            } catch (\Throwable $e) {
    ...
    protected function initialRequest(): PromiseInterface
    {
        ...
        return $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) use ($command) {
    ...
    protected function computeObjectSizeFromContentRange(string $contentRange): int
    {
        ...
        throw new S3TransferException("Invalid content range \"{$contentRange}\"");
    }
    /**
    ...
    private function downloadInitiated(array $commandArgs): void
    {
        if ($this->currentSnapshot === null) {
            $this->currentSnapshot = new TransferProgressSnapshot($commandArgs['Key'], 0, $this->objectSizeInBytes);
        } else {
            $this->currentSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes(), $this->currentSnapshot->getTotalBytes(), $this->currentSnapshot->getResponse());
        }
        $this->listenerNotifier?->transferInitiated([AbstractTransferListener::REQUEST_ARGS_KEY => $commandArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
    ...
    private function downloadFailed(\Throwable $reason): void
    {
        ...
        $this->currentSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes(), $this->currentSnapshot->getTotalBytes(), $this->currentSnapshot->getResponse(), $reason);
        $this->listenerNotifier?->transferFail([AbstractTransferListener::REQUEST_ARGS_KEY => $this->downloadRequestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot, 'reason' => $reason]);
    }
    /**
    ...
    private function partDownloadCompleted(ResultInterface $result, array $requestArgs): void
    {
        ...
        $newSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes() + $partDownloadBytes, $this->objectSizeInBytes, $result->toArray());
        $this->currentSnapshot = $newSnapshot;
        $this->listenerNotifier?->bytesTransferred([AbstractTransferListener::REQUEST_ARGS_KEY => $requestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
    ...
    {
        $newSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes(), $this->objectSizeInBytes, $this->currentSnapshot->getResponse());
        $this->currentSnapshot = $newSnapshot;
        $this->listenerNotifier?->transferComplete([AbstractTransferListener::REQUEST_ARGS_KEY => $this->downloadRequestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
