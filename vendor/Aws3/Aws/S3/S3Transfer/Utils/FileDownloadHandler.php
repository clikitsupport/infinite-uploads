...
namespace ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Utils;

use ClikIT\Infinite_Uploads\Aws\S3\S3Transfer\Exception\FileDownloadException;
...
final class FileDownloadHandler extends AbstractDownloadHandler
{
    ...
    public function transferInitiated(array $context): void
    {
        if ($this->failsWhenDestinationExists && file_exists($this->destination)) {
            throw new FileDownloadException("The destination '{$this->destination}' already exists.");
        } elseif (is_dir($this->destination)) {
            throw new FileDownloadException("The destination '{$this->destination}' can't be a directory.");
        }
    ...
    public function bytesTransferred(array $context): bool
    {
        ...
        file_put_contents($this->temporaryDestination, $partBody, \FILE_APPEND);
        return \true;
    }
    /**
     * @param array $context
    ...
    public function transferComplete(array $context): void
    {
        ...
        if (!rename($this->temporaryDestination, $this->destination)) {
            throw new FileDownloadException("Unable to rename the file `{$this->temporaryDestination}` to `{$this->destination}`.");
        }
    }
    /**
     * @param array $context
    ...
    public function transferFail(array $context): void
    {
        ...
        } elseif (file_exists($this->destination) && !str_contains($context[self::REASON_KEY], "The destination '{$this->destination}' already exists.")) {
            unlink($this->destination);
        }
    }
    /**
