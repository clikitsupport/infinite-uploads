...
abstract class AbstractMultipartUploader implements PromisorInterface
{
    ...
    protected function validateConfig(array &$config): void
    {
        ...
        if ($partSize < self::PART_MIN_SIZE || $partSize > self::PART_MAX_SIZE) {
            throw new \InvalidArgumentException("Part size config must be between " . self::PART_MIN_SIZE . " and " . self::PART_MAX_SIZE . " bytes " . "but it is configured to {$partSize}");
        }
    }
    /**
     * @return string|null
    ...
    protected function abortMultipartOperation(): PromiseInterface
    {
        ...
        $command = $this->s3Client->getCommand('AbortMultipartUpload', $abortMultipartUploadArgs);
    ...
    protected function collectPart(ResultInterface $result, CommandInterface $command): void
    {
        ...
        $partData = ['PartNumber' => $command['PartNumber'], 'ETag' => $checksumResult['ETag']];
    ...
    {
        return (new CommandPool($this->s3Client, $commands, ['concurrency' => $this->config['concurrency'], 'fulfilled' => $fulfilledCallback, 'rejected' => $rejectedCallback]))->promise();
    }
    /**
     * @param array $requestArgs
     * @return void
    ...
    protected function operationInitiated(array $requestArgs): void
    {
        if ($this->currentSnapshot === null) {
            $this->currentSnapshot = new TransferProgressSnapshot($requestArgs['Key'], 0, $this->getTotalSize());
        }
        $this->listenerNotifier?->transferInitiated([AbstractTransferListener::REQUEST_ARGS_KEY => $requestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
     * @param ResultInterface $result
     * @return void
    ...
    {
        $newSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes(), $this->currentSnapshot->getTotalBytes(), $result->toArray(), $this->currentSnapshot->getReason());
        $this->currentSnapshot = $newSnapshot;
        $this->listenerNotifier?->transferComplete([AbstractTransferListener::REQUEST_ARGS_KEY => $this->requestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
    ...
    protected function operationFailed(Throwable $reason): void
    {
        ...
        if ($this->currentSnapshot === null) {
            $this->currentSnapshot = new TransferProgressSnapshot('Unknown', 0, 0);
        }
        ...
        if (!empty($this->uploadId)) {
            trigger_error("Multipart Upload with id: " . $this->uploadId . " failed");
            $this->abortMultipartOperation()->wait();
        }
        $this->listenerNotifier?->transferFail([AbstractTransferListener::REQUEST_ARGS_KEY => $this->requestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot, 'reason' => $reason]);
    }
    /**
     * @param int $partSize
     * @param array $requestArgs
    ...
    {
        $newSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes() + $partSize, $this->currentSnapshot->getTotalBytes(), $this->currentSnapshot->getResponse(), $this->currentSnapshot->getReason());
        $this->currentSnapshot = $newSnapshot;
    ...
    {
        return max($this->getTotalSize() / self::PART_MAX_NUM, $this->config['target_part_size_bytes']);
    }
}
