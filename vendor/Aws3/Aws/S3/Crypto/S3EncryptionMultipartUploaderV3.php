...
use ClikIT\Infinite_Uploads\Aws\Crypto\MaterialsProviderInterfaceV3;
...
class S3EncryptionMultipartUploaderV3 extends MultipartUploader
{
    ...
    public function __construct(S3ClientInterface $client, $source, array $config = [])
    {
        ...
        $this->algorithmSuite = AlgorithmSuite::validateCommitmentPolicyOnEncrypt($cipherOptions, $this->keyCommitmentPolicy);
        //= ../specification/s3-encryption/client.md#optional-api-operations
        //= type=implication
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = Promise\Create::promiseFor($this->encrypt($this->source, $this->algorithmSuite, $this->config ?: [], $this->provider, $envelope))->then(function ($bodyStream) use ($envelope) {
