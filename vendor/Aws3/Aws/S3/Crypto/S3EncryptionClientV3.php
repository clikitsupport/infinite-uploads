...
use ClikIT\Infinite_Uploads\Aws\Crypto\DecryptionTraitV3;
...
use ClikIT\Infinite_Uploads\Aws\Crypto\AbstractCryptoClientV3;
use ClikIT\Infinite_Uploads\Aws\Crypto\EncryptionTraitV3;
use ClikIT\Infinite_Uploads\Aws\Crypto\MetadataEnvelope;
use ClikIT\Infinite_Uploads\Aws\Crypto\AlgorithmSuite;
...
class S3EncryptionClientV3 extends AbstractCryptoClientV3
{
    ...
    )
    {
        ...
        if ($client instanceof S3EncryptionClientV3) {
            throw new CryptoException("Client configuration error." . " An S3 Encryption Client is not a valid S3 client for an S3 Encryption Client.");
        }
        $this->client = $client;
        //= ../specification/s3-encryption/client.md#instruction-file-configuration
        //= type=implication
        ...
        MetricsBuilder::appendMetricsCaptureMiddleware($this->client->getHandlerList(), MetricsBuilder::S3_CRYPTO_V3);
    ...
    public function putObjectAsync(array $args): PromiseInterface
    {
        ...
        $algorithmSuite = AlgorithmSuite::validateCommitmentPolicyOnEncrypt($cipherOptions, $keyCommitmentPolicy);
        ...
        return Promise\Create::promiseFor(
            ...
            $this->encrypt(Psr7\Utils::streamFor($args['Body']), $algorithmSuite, $options, $provider, $envelope)
    ...
    public function getObjectAsync(array $args): PromiseInterface
    {
        ...
        if (!isset($args['@SecurityProfile']) || !in_array($args['@SecurityProfile'], self::SUPPORTED_SECURITY_PROFILES)) {
            throw new CryptoException("@SecurityProfile is required and must be" . " set to 'V3' or 'V3_AND_LEGACY'");
        }
        // Only throw this legacy warning once per client
        if (in_array($args['@SecurityProfile'], self::LEGACY_SECURITY_PROFILES) && $this->legacyWarningCount < 1) {
        ...
        $promise = $this->client->getObjectAsync($args)->then(function ($result) use ($provider, $instructionFileSuffix, $strategy, $keyCommitmentPolicy, $args) {
            if ($strategy === null) {
            ...
            $result['Body'] = $this->decrypt($result['Body'], $provider, $envelope, $keyCommitmentPolicy, $args);
            return $result;
