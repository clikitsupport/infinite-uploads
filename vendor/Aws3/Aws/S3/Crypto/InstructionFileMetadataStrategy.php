...
class InstructionFileMetadataStrategy implements MetadataStrategyInterface
{
    ...
    public function save(MetadataEnvelope $envelope, array $args)
    {
        ...
        if (MetadataEnvelope::isV3Envelope($envelope)) {
            ...
            if (!isset($envelope[MetadataEnvelope::ENCRYPTED_DATA_KEY_V3]) || !isset($envelope[MetadataEnvelope::ENCRYPTED_DATA_KEY_ALGORITHM_V3]) || !isset($envelope[MetadataEnvelope::ENCRYPTION_CONTEXT_V3])) {
    ...
    public function load(array $args)
    {
        ...
        foreach ($constantValues as $constant) {
            if (isset($metadataHeaders[$constant])) {
                ...
                if (empty($envelope[$constant])) {
                    ...
                    throw new CryptoException("Duplicate keys are not allowed" . " in Instruction Files. ");
                }
            }
        }
        ...
        if (!empty($envelope[MetadataEnvelope::ENCRYPTED_DATA_KEY_V3])) {
            ...
            if (!empty($envelope[MetadataEnvelope::CONTENT_CIPHER_V3]) || !empty($envelope[MetadataEnvelope::KEY_COMMITMENT_V3]) || !empty($envelope[MetadataEnvelope::MESSAGE_ID_V3])) {
                throw new CryptoException("One or more reserved keys found in" . " Instruction file when they should not be present.");
            }
            ...
            if (!MetadataEnvelope::isV3Envelope($envelope)) {
                throw new CryptoException("Expected a V3 envelope but was unable to" . " constuct one.");
            }
            return $envelope;
        }
        // if we are not reading a v3 object then it must be a v2 object
        if (!MetadataEnvelope::isV2Envelope($envelope) && !MetadataEnvelope::isV1Envelope($envelope)) {
