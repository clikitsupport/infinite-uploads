...
{
    public const ALLOW_LISTED_SERVICES = ['signin' => \true];
    ...
    public function __construct(string $serviceName)
    {
        if (!extension_loaded(self::EXT_OPENSSL)) {
            throw new \RuntimeException('the `openssl` extension is required to generate DPop signatures. ' . 'Please install or enable the `openssl` extension.');
        }
        if (!isset(self::ALLOW_LISTED_SERVICES[$serviceName])) {
            throw new \InvalidArgumentException("The '{$serviceName}' service does not support DPop signatures. " . 'Please configure a signature version this service supports.');
        }
    }
    /**
    ...
    private function generateDpopProof(\OpenSSLAsymmetricKey $key, UriInterface $uri): string
    {
        ...
        if (($keyDetails['ec']['curve_name'] ?? '') !== self::CURVE_NAME) {
            throw new \InvalidArgumentException('DPoP signature keys must use P-256 curve. ' . 'Please check your configuration and try again.');
        }
        ['x' => $x, 'y' => $y] = $keyDetails['ec'];
        $header = $this->buildDpopHeader($x, $y);
        $payload = $this->buildDpopPayload((string) $uri);
        $message = $this->base64url_encode(json_encode($header)) . '.' . $this->base64url_encode(json_encode($payload));
        $signature = '';
        if (!openssl_sign($message, $signature, $key, \OPENSSL_ALGO_SHA256)) {
            $error = openssl_error_string();
            throw new \RuntimeException('Failed to generate signature.' . ($error ? ": {$error}" : '.'));
        }
    ...
    {
        return array_merge_recursive(self::HEADER_SCHEMA, ['jwk' => ['x' => $this->base64url_encode($x), 'y' => $this->base64url_encode($y)]]);
    }
    /**
    ...
    {
        return array_merge(self::PAYLOAD_SCHEMA, ['jti' => $this->uuidv4(), 'htu' => $uri, 'iat' => time()]);
    }
    /**
