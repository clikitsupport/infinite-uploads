<?php
namespace ClikIT\Infinite_Uploads\Aws\Identity\S3;
...
class S3ExpressIdentityProvider
{
    ...
    public function __invoke($command)
    {
        ...
        $identity = new ClikIT\Infinite_Uploads\Aws\Identity\S3\S3ExpressIdentity(
    ...
    private function getS3Client()
    {
        if (is_null($this->s3Client)) {
            $this->s3Client = $this->config['client']
                ?? new ClikIT\Infinite_Uploads\Aws\S3\S3Client([
                    'region' => $this->region,
