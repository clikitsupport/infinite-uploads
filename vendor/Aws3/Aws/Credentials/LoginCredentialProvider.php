...
use ClikIT\Infinite_Uploads\Aws\Signin\SigninClient;
use ClikIT\Infinite_Uploads\Aws\Signin\Exception\SigninException;
...
final class LoginCredentialProvider
{
        ...
        return Promise\Coroutine::of(function () {
            ...
            if ($this->shouldRefresh($credentials)) {
                try {
                    ...
                    throw new CredentialsException('Unable to refresh login credentials: ' . $e->getAwsErrorMessage(), $e->getCode(), $e);
                } catch (\Exception $e) {
                    // For other refresh failures, log and continue with existing token
                    trigger_error('Continuing with existing token after refresh failure: ' . $e->getMessage(), \E_USER_NOTICE);
                }
            }
            yield $credentials;
        });
    ...
    private function refresh(Credentials $currentCredentials): \Generator
    {
        ...
        try {
            $refreshed = (yield $this->client->createOAuth2TokenAsync([self::REQUEST_KEY_TOKEN_INPUT => [self::REQUEST_KEY_CLIENT_ID => $this->token[self::REQUEST_KEY_CLIENT_ID], self::REQUEST_KEY_GRANT_TYPE => self::GRANT_TYPE, self::REQUEST_KEY_REFRESH_TOKEN => $this->token[self::REQUEST_KEY_REFRESH_TOKEN]], self::KEY_DPOP_KEY => $this->token[self::KEY_DPOP_KEY]]))->get(self::RESULT_TOKEN_OUTPUT);
            $newCredentials = self::createCredentials($refreshed[self::KEY_ACCESS_TOKEN], time() + $refreshed[self::RESULT_EXPIRES_IN], $currentCredentials->getAccountId());
        ...
        try {
            ...
            trigger_error('Failed to update credential cache during refresh: ' . $e->getMessage() . '. Using refreshed credentials in memory.', \E_USER_NOTICE);
        }
    ...
    private function getExternalRefresh(Credentials $currentCredentials): ?array
    {
        try {
            ...
            if ($latestToken[self::REQUEST_KEY_REFRESH_TOKEN] === $this->token[self::REQUEST_KEY_REFRESH_TOKEN]) {
                return null;
            }
            // Expiration must be newer
            if ($latestCredentials->getExpiration() <= $currentCredentials->getExpiration()) {
                return null;
            }
    ...
    {
        $credentials = $this->token[self::KEY_ACCESS_TOKEN];
        $updates = [self::KEY_ACCESS_TOKEN => [self::KEY_ACCESS_KEY_ID => $credentials->getAccessKeyId(), self::KEY_SECRET_ACCESS_KEY => $credentials->getSecretKey(), self::KEY_SESSION_TOKEN => $credentials->getSecurityToken(), self::KEY_ACCOUNT_ID => $credentials->getAccountId(), self::KEY_EXPIRES_AT => gmdate('Y-m-d\TH:i:s\Z', $credentials->getExpiration())], self::REQUEST_KEY_REFRESH_TOKEN => $this->token[self::REQUEST_KEY_REFRESH_TOKEN]];
        $existing = json_decode(file_get_contents($this->tokenLocation), \true, 512, \JSON_THROW_ON_ERROR);
        $merged = array_merge($existing, $updates);
        $result = file_put_contents($this->tokenLocation, json_encode($merged, \JSON_THROW_ON_ERROR), \LOCK_EX);
        if (!$result) {
    ...
    {
        try {
            $cached = json_decode(file_get_contents($this->tokenLocation), \true, 512, \JSON_THROW_ON_ERROR);
        } catch (\JsonException $e) {
            throw new CredentialsException('Invalid JSON in cache file: ' . $e->getMessage(), 0, $e);
        }
        if (self::hasAllRequiredKeys($cached, self::REQUIRED_CACHE_KEYS) && self::hasAllRequiredKeys($cached[self::KEY_ACCESS_TOKEN] ?? [], self::REQUIRED_ACCESS_TOKEN_KEYS)) {
            // Convert expiresAt to Unix timestamp
            $expiresAt = strtotime($cached[self::KEY_ACCESS_TOKEN][self::KEY_EXPIRES_AT]);
            if ($expiresAt === \false) {
                throw new CredentialsException('Invalid expiration date format in cached token `' . self::KEY_EXPIRES_AT . '` field.' . self::REAUTHENTICATE_MSG);
            }
            $cached[self::KEY_ACCESS_TOKEN] = self::createCredentials($cached[self::KEY_ACCESS_TOKEN], $expiresAt, $cached[self::KEY_ACCESS_TOKEN][self::KEY_ACCOUNT_ID]);
            // Load DPoP key
            $cached[self::KEY_DPOP_KEY] = openssl_pkey_get_private($cached[self::KEY_DPOP_KEY]);
            if ($cached[self::KEY_DPOP_KEY] === \false) {
                $error = openssl_error_string();
                throw new CredentialsException('Failed to load DPoP private key from cached token for profile ' . "{$this->profileName}: " . ($error ? ": {$error}" : '.') . self::REAUTHENTICATE_MSG);
            }
            return $cached;
        }
        throw new CredentialsException('Missing required keys in cached token for profile ' . $this->profileName . '.' . self::REAUTHENTICATE_MSG);
    }
    /**
    ...
    private function handleRefreshException(\Exception $e): \Exception
    {
        if ($e instanceof SigninException) {
            trigger_error('Failed to refresh login credentials: ' . $e->getAwsErrorMessage(), \E_USER_NOTICE);
            if ($e->getAwsErrorCode() === 'AccessDeniedException') {
                $error = strtolower($e->get('error'));
                switch ($error) {
                    case 'token_expired':
                        return new CredentialsException('Your session has expired.' . self::REAUTHENTICATE_MSG, 0, $e);
                    case 'user_credentials_changed':
                        return new CredentialsException('Unable to refresh credentials because of a change in your password. ' . 'Please reauthenticate with your new password.', 0, $e);
                    case 'insufficient_permissions':
                        return new CredentialsException('Unable to refresh credentials due to insufficient permissions. ' . 'You may be missing permission for the `CreateOAuth2Token` action.', 0, $e);
                }
            }
            return $e;
        }
        // For all other exceptions
        trigger_error('Unexpected error refreshing login credentials: ' . $e->getMessage(), \E_USER_NOTICE);
        return $e;
    }
    /**
    ...
    private function resolveTokenLocation(string $profileName): string
    {
        ...
        if (!is_readable($configFile)) {
            throw new CredentialsException('Unable to load configuration file at ' . $configFile . '. Please ensure the file exists at the specified location.');
        }
        ...
        if ($profileName === self::PROFILE_DEFAULT) {
            ...
            $profileData = $profiles[$profileName] ?? $profiles[self::PROFILE_SECONDARY . $profileName] ?? null;
        }
        if (!$profileData) {
            throw new CredentialsException("Profile '{$profileName}' does not exist. " . "Please ensure the specified profile is set at {$configFile}.");
        }
        if (empty($session = $profileData[self::KEY_PROFILE_LOGIN_SESSION] ?? null)) {
            throw new CredentialsException("Profile '{$profileName}' did not contain a " . self::KEY_PROFILE_LOGIN_SESSION . " value. " . 're-authentication using `aws login` may be needed.');
        }
        // Resolve location and ensure it exists
        $cacheDirectory = getenv(self::ENV_CACHE_DIRECTORY) ?: CredentialProvider::getHomeDir() . self::DEFAULT_CACHE_DIRECTORY;
        $cacheFile = $cacheDirectory . \DIRECTORY_SEPARATOR . hash('sha256', trim($session)) . '.json';
        if (!@is_readable($cacheFile)) {
            throw new CredentialsException("Failed to load cached credentials for profile " . "'{$profileName}'." . self::REAUTHENTICATE_MSG);
        }
        return $cacheFile;
    }
    /**
    ...
    {
        $resolvedRegion = $region ?? ConfigurationResolver::env(self::KEY_CLIENT_REGION) ?? ConfigurationResolver::ini(self::KEY_CLIENT_REGION, 'string', $profile) ?? ConfigurationResolver::ini(self::KEY_CLIENT_REGION, 'string', self::PROFILE_SECONDARY . $profile);
        if (empty($resolvedRegion)) {
            throw new CredentialsException('Unable to determine region for the Sign-In service client ' . ' used for refreshing Login credentials. You can provide a region in-code ' . 'when constructing the provider, by setting the AWS_REGION environment variable' . ', or by setting a `region` in your specified profile.');
        }
        return new SigninClient([self::KEY_CLIENT_REGION => $resolvedRegion, self::KEY_CLIENT_SIGNATURE_VERSION => self::CLIENT_SIGNATURE_DPOP, self::KEY_CLIENT_CREDENTIALS => \false]);
    }
    /**
    ...
    {
        return new Credentials($tokenData[self::KEY_ACCESS_KEY_ID], $tokenData[self::KEY_SECRET_ACCESS_KEY], $tokenData[self::KEY_SESSION_TOKEN], $expiration, $accountId, CredentialSources::PROFILE_LOGIN);
    }
    /**
