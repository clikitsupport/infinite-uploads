...
use ClikIT\Infinite_Uploads\Aws\Kms\KmsClient;
/**
...
class KmsMaterialsProviderV3 extends MaterialsProviderV3 implements MaterialsProviderInterfaceV3
{
    ...
    public function decryptCek(string $encryptedCek, array $materialDescription, array $options): string
    {
        ...
        if (!isset($options['@kmsencryptioncontext'])) {
            ...
            if (!is_array($options['@kmsencryptioncontext'])) {
                throw new CryptoException("'When using @KmsMaterialsProviderV3, it" . " must be an associative array (or empty array).");
            }
            ...
            if (isset($options['@kmsencryptioncontext']['kms_cmk_id'])) {
                throw new CryptoException("Conflict in reserved @KmsEncryptionContext" . " key kms_cmk_id. This value is reserved for the S3" . " Encryption Client and cannot be set by the user.");
            }
        ...
        $params = ['CiphertextBlob' => $encryptedCek, 'EncryptionContext' => $encryptionContext];
    ...
    public function generateCek($keySize, $context, $options): array
    {
        ...
        if (!isset($options['@kmsencryptioncontext']) || !is_array($options['@kmsencryptioncontext'])) {
            throw new CryptoException("'@KmsEncryptionContext' is a" . " required argument when using KmsMaterialsProviderV3, and" . " must be an associative array (or empty array).");
        }
        if (isset($options['@kmsencryptioncontext']['aws:x-amz-cek-alg'])) {
            throw new CryptoException("Conflict in reserved @KmsEncryptionContext" . " key aws:x-amz-cek-alg. This value is reserved for the S3" . " Encryption Client and cannot be set by the user.");
        }
